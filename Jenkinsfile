    ...
    stages {
        stage('PLAY ANSIBLE BOOK..') {
             steps {
                ...
                sh 'pip install boto'
                sh 'ansible ALL=(ALL) NOPASSWD:ALL && sudo su - ansible && ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook ansible/playbook/playbook.yml -i ansible/inventory/hosts/ec2.py -vvvvv' 
            }
        }   
    }
}