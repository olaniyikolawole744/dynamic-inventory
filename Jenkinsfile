    ...
    stages {
        stage('PLAY ANSIBLE BOOK..') {
             steps {
                ...
                sh 'pip install boto'
                sh 'ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook --private-key /tmp/tf-packer -u ansible ansible/playbook/playbook.yml -i ansible/inventory/hosts/ec2.py -C' 
            }
        }   
    }
}